...
function Checkout() {
  ...
  const [isMockPayment, setIsMockPayment] = useState(false); // Mock payment state
  const paymentDetails = {
  ...
  async function handleSubscription(e) {
    ...
    if (isMockPayment) {
      ...
      // Razorpay integration (if not in mock mode)
      const options = {
        key: razorpayKey,
        subscription_id: subscription_id,
        name: "Coursify Pvt. Ltd.",
  ...
  return (
    <HomeLayout>
      ...
      <div className="mt-4 text-center">
        <button
          onClick={() => setIsMockPayment(!isMockPayment)}
          className="text-sm text-blue-500 hover:underline"
        >
          {isMockPayment ? "Switch to real payment" : "Switch to mock payment"}
        </button>
      </div>
    </HomeLayout>
  );
